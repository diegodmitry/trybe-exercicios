...
class Student extends Person {
  private _enrollment: string; // matricula da estudante
  ...
  set setExamsGrades(grades: number[]) {
    if (grades.length !== 4) throw new Error("You should put 4 grades.");
    this._examsGrades = grades;
  }

  set setWorksGrades(grades: number[]) {
    if (grades.length !== 2) throw new Error("You should put 2 grades.");
